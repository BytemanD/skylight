            ...
            <template v-slot:content>
                ...
                <v-col cols="12" :hidden="displayType != 0">
                    <v-data-table density='compact' :loading="table.loading" :headers="table.headers"
                        :items="table.azMap[table.zoneName].hosts" :items-per-page="table.itemsPerPage"
                        ...
                        <template v-slot:top>
                            <v-row>
                                ...
                                <v-col>
                                    <v-text-field small density='compact' single-line hide-details
                                        v-model="table.search" label="搜索"></v-text-field>
                                </v-col>
                                <v-col cols="1" class="text-center">
                                    <v-btn color="info" icon="mdi-refresh" variant="text"
                                        v-on:click="table.refresh()"></v-btn>
                                </v-col>
                            </v-row>
                        </template>
