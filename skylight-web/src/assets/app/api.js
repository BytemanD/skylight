...
class System extends Restfulclient {
    ...
    async login(cluster, project, username, password){
        let auth = {
            cluster: cluster,
            project: project,
            user: username,
...
class ComputeService extends Restfulclient {
    ...
    async disable(id, disabledReason=null) {
        let data = { status: 'disabled' }
        if (disabledReason){
            data.disabled_reason = disabledReason
        }
        return (await this.put(`${id}`, data)).service
    }
    async enable(id) {
    ...
    async find(host, binary) {
        let fileter = {host: host, binary: binary}
        let services = (await this.list(fileter)).services
...
class User extends Restfulclient {
    ...
    async getByName(name) {
        let users = (await this.list({name: name})).users
        if (users.length == 0) {
...
class Image extends Restfulclient {
    ...
    async replaceProperties(id, data) {
        return await this.patch(id, data, { 'Content-Type': 'application/openstack-images-v2.1-json-patch' })
    }
...
class Cluster extends Restfulclient {
    constructor() { super('/clusters') }

    async add(data) {
        return this.post({ cluster: data })
    }
    async regions(clusterId) {
        return (await this.get(`/clusters/${clusterId}/regions`)).regions
    }
}
